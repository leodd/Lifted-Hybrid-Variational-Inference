...
from OneShot import OneShot, LiftedOneShot2
...
for _ in range(num_tests):
    ...
    lifting_reg_coef = 0.1
    T = 30
    osi = LiftedOneShot2(g=g, cg=cg, K=K, T=T, seed=seed,
                         lifting_reg_coef=lifting_reg_coef)  # can be moved outside of all loops if the ground MRF doesn't change
    for j in range(num_runs):
        start_time = time.process_time()
        osi.run(lr=lr, its=its, fix_mix_its=fix_mix_its)
        time_cost[name] = (time.process_time() - start_time) / num_runs / num_tests + time_cost.get(name, 0)
