...
for i in range(num_test):
    ...
    for idx, rv in enumerate(rvs_table[t - 1]):
        ...
        # val = list()
        # val2 = list()
        # integral_points = np.linspace(-10, 10, 100)
        # for x in integral_points:
        #     val.append(infer.belief(x, rv))
        #     val2.append(ans2.belief(x, rv))
        # plt.plot(integral_points, val)
        # plt.plot(integral_points, val2)
        # plt.show()
